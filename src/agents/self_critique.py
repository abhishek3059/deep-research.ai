...
class SelfCritiqueAgent:
    ...
    async def critique(self, state: AgentState) -> AgentState:
        ...
        messages = self._critique_messages(state.query, state.current_answer, context)
        raw = await self._llm.generate(messages, temperature=0.0)

        state.critique_result = self._parse_critique(raw)
        state.critique = state.critique_result.to_state_text()
        state.iteration_count += 1
        state.status = self._route_after_critique(state)

        logger.info(
            "Critique complete",
            iteration=state.iteration_count,
            passed=state.critique_result.passed,
            next_status=state.status,
        )
        return state

    async def revise(self, state: AgentState) -> AgentState:
        """Rewrite the answer to address the recorded critique."""
        context = self._format_context(self._merge_results(state.research_results))
        messages = self._revise_messages(
            state.query, state.current_answer, state.critique, context
        )
    ...
    async def step(self, state: AgentState) -> AgentState:
        ...
        if state.status == "critique":
            return await self.critique(state)
    ...
    async def run(
        ...
        Returns:
            A :class:`SelfCritiqueResult` describing the delivered answer.
        """
        state = AgentState(
            query=query,
        ...
        while state.status != "deliver":
            ...
            steps += 1
            if steps > guard:
                raise RuntimeError("Self-critique loop exceeded its guard limit")
    ...
    def _to_result(self, state: AgentState) -> SelfCritiqueResult:
    ...
    def _revise_messages(
        self, query: str, answer: str, critique: str, context: str
    ) -> list[dict[str, str]]:
    ...
    def _parse_critique(self, raw: str) -> CritiqueResult:
        ...
        if payload is None:
            return self._fallback_critique(raw)
    ...
    def _normalize_dimensions(self, value: Any) -> list[CritiqueDimension]:
        ...
        for name, spec in entries:
            if isinstance(spec, dict):
                ...
                dimensions.append(
                    CritiqueDimension(name=name, passed=bool(spec), issue="")
                )
        return dimensions

    @staticmethod
    def _fallback_critique(raw: str) -> CritiqueResult:
        ...
        return CritiqueResult(passed=passed, summary=raw.strip()[:500], raw=raw)
